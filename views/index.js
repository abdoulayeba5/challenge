...
mongoose.connect("mongodb://127.0.0.1:27017/Netcode", {
      ...
      if (today < date) {
        ...
        const data = await db
          ...
          .find({
            ...
            ...filter,
            date: datta[0].date,
            matricule: parseInt(req.params.id),
          })
          .toArray();
        ...
        if (data.length == 1) {
          ...
          console.log(niveau);
          var types =
            action === "SN" ? ["NSN", "NCC"] : action === "SR" ? ["NSR"] : [];
          const mat = req.params.id;
          const dd = datta[0].date + " " + datta[0].time + ":00";
          return res.render("env_reclamation1", {
            users: result,
            nivea: niveau,
            matricule: mat,
            reclamation: "open",
    ...
    if (user) {
      ...
      bcrypt.compare(password, user.password, (err, passwordMatch) => {
        ...
        if (passwordMatch) {
          ...
          return res.render("login", {
            Eror: "Adresse e-mail ou mot de passe incorrect.",
          });
        }
      });
    } else {
...
app.get("/date_open_prof", (req, res) => {
  ...
  db.collection("enseignant")
    .find({}, { projection: { autresprof: 1, email: 1 } })
    .toArray((err, enseignants) => {
      if (err) {
